/*class MyHomePage extends StatefulWidget {
  const MyHomePage({super.key, required this.title});

  final String title;

  @override
  State<MyHomePage> createState() => _MyHomePageState();
}

class _MyHomePageState extends State<MyHomePage> {
  int _counter = 0;

  void _incrementCounter() {
    setState(() {
      _counter++;
    });
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        backgroundColor: Theme.of(context).colorScheme.inversePrimary,
        title: Text(widget.title),
      ),
      body: Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: <Widget>[
            const Text(
              'You have pushed the button this many times:',
            ),
            Text(
              '$_counter',
              style: Theme.of(context).textTheme.headlineMedium,
            ),
          ],
        ),
      ),
      floatingActionButton: FloatingActionButton(
        onPressed: _incrementCounter,
        tooltip: 'Increment',
        child: const Icon(Icons.add),
      ), // This trailing comma makes auto-formatting nicer for build methods.
    );
  }
}*/

import 'package:flutter/material.dart';
import 'package:get/get.dart';
import 'package:testing_user_list_with_api/app/core/theme/app_colors.dart';
import 'package:testing_user_list_with_api/app/core/theme/app_text_theme.dart';
import 'package:testing_user_list_with_api/app/presentation/manager/controllers/user_details_controller.dart';
import 'package:testing_user_list_with_api/app/presentation/manager/controllers/user_list_controller.dart';
import 'package:testing_user_list_with_api/app/presentation/widgets/app_circular_image_widget.dart';
import 'package:testing_user_list_with_api/app/presentation/widgets/app_loading_overlay_widget.dart';
import 'package:testing_user_list_with_api/app/presentation/widgets/app_parent_widget.dart';
import 'package:testing_user_list_with_api/app/presentation/widgets/app_text_widget.dart';
import 'package:testing_user_list_with_api/app/presentation/widgets/custom_button_widget.dart';

import '../../manager/routes/app_navigators.dart';

class UserListPage extends StatelessWidget {
  UserListPage({super.key});

  final controller = Get.put(UserListController());
  final detailsController = Get.put(UserDetailsController());

  @override
  Widget build(BuildContext context) => AppParentWidget(
        safeAreaTop: false,
        resizeToAvoidBottomInset: true,
        bottomWidget: SizedBox(
          width: Get.width,
          height: 46.0,
          child: Obx(() {
            return AppButton(
              label: controller.buttonText.value ?? "Load Next",
              labelColor: textColor,
              bgColor: accentColor,
              width: Get.width,
              onTap: () {
                controller.getNextData(controller: controller);
              },
            );
          }),
        ),
        bodyWidget: Obx(
          () => LoadingOverlay(
            isLoading: controller.isLoading.value,
            child: Obx(
              () {
                return ListView.builder(
                  itemCount: controller.ulm.value.length,
                  itemBuilder: (BuildContext ctx, int index) => Obx(() {
                    return InkWell(
                      onTap: () {
                        AppNavigator.toUserDetailsPage();
                        int? id = controller.ulm.value[index].data![index].id;
                        detailsController.getData(index: id);
                        detailsController.update();
                      },
                      child: ListTile(
                        leading: AppCircularImage(
                          url: controller.ulm.value[index].data?[index].avatar,

                          //url: "data:image/jpeg;base64,",
                        ),
                        title: AppText(
                          label:
                              "${controller.ulm.value[index].data?[index].firstName}",
                          textStyle: textStyle(
                              size: 16.0, overflow: TextOverflow.ellipsis),
                        ),
                      ),
                    );
                  }),
                );
              },
            ),
          ),
        ),
      );
}
